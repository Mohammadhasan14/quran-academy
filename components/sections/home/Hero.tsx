...
export function Hero() {
  ...
  return (
    <div className="mx-auto flex max-w-[1180px] flex-col gap-10 px-4 pt-12 pb-12 sm:px-6 sm:pt-16 sm:pb-14 lg:flex-row lg:items-center lg:gap-16 lg:px-8 lg:pt-[84px] lg:pb-[72px]">
      <div className="min-w-0 flex-[1.15]">
        <div className="mb-5 inline-flex items-center gap-2 text-xs tracking-[0.16em] text-gold uppercase">
          <span className="h-1.5 w-1.5 flex-none rotate-45 bg-gold" />
          A school, not an app — enrollment open
        </div>
        <h1 className="m-0 text-4xl leading-[1.15] font-extrabold tracking-[-0.01em] text-balance sm:text-[44px] lg:text-[52px] lg:leading-[1.12]">
          Learn the art of Quran recitation from a master Qari.
        </h1>
        <p className="mt-5 max-w-[540px] text-base leading-[1.55] text-[#9FAAA3] sm:text-[19px]">
      ...
      <div className="w-full min-w-0 flex-[0.85]">
        <div className="rounded-[18px] border border-gold/[.22] bg-[#131D19] px-5 py-6 sm:px-[30px] sm:py-7">
          ...
          <div className="flex gap-2.5 sm:gap-3.5">
            <button
              onClick={() => setHeard("7")}
              className="min-w-0 flex-1 cursor-pointer rounded-2xl border px-0 pt-[18px] pb-3.5 text-center hover:border-gold/60"
              style={{
                borderColor: heard === "7" ? "rgba(203,165,92,.6)" : "rgba(237,231,216,.12)",
                background: heard === "7" ? "rgba(203,165,92,.08)" : "transparent",
              }}
            >
              <div className="font-arabic-ui text-[34px] leading-none text-[#EDE7D8] sm:text-[40px]">ح</div>
            ...
            >
              <div className="font-arabic-ui text-[34px] leading-none text-[#EDE7D8] sm:text-[40px]">ه</div>
